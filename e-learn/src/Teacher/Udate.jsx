...
const UpdateCourse = () => {
  ...
  useEffect(() => {
    const url = `http://localhost:5010/api/course/${id}`;
  ...
  const PostData = async (e) => {
    ...
    const res = await fetch(`http://localhost:5010/api/course/update/${id}`, {
      method: "PUT",
      headers: {
  ...
  const deleteData = async () => {
    const res = await fetch(`http://localhost:5010/api/course/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
